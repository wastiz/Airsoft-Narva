(function($) {
	...
	var url = proxyApi
	var access_token = 'a671bfa8a671bfa8a671bfa844a60ab3b1aa671a671bfa8c78256e6912a43a0e011f5ef'
	$(function() {
		...
		$[nps] = {
			...
			link_mapper: function(el) {
				return [
					el.href, '<a href="' + el.href + '">' + el.title + '</a>'
				]
			}
		};
		...
		$.fn[nps] = function(opts) {
			...
			function showAlbum() {
				 ...
				 */
				var query = url + '?&photo_sizes=1&extended=1&album_id=' + res[2] + '&access_token=' + access_token + '&count=1000' + '&owner_id=' + res[1] + '&rev=' + meta_opts.rev
					/**
					 * Version of VK API
					 */
					+ '&v=5.131&callback=?';
				if(meta_opts.width < 0) {
					return;
				}
						...
						if(i % 2 === 0) {
							if(i <= (result.length - 2)) {
								if(result[i] > 3 && result[i + 1] > 3) {
									var max = (result[i] / 3 ^ 0) < (result[i + 1] / 3 ^ 0) ? result[i] / 3 ^ 0 : result[i + 1] / 3 ^ 0,
				 ...
				 */
				if(!json) {
					$.getJSON(query, renderAlbumList).fail(function() {
